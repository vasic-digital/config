...
echo "=== Config Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: CONFIG_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== Config Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Config Stress Challenge: PASSED ==="
