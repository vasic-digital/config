...
# Config per CONST-035 + CONST-050(B). Submodule cascade per
# CONST-051(A). Targets $CONFIG_HEALTH_URL.
...
echo "=== Config DDoS Health-Flood Challenge ==="
echo "  url=$HEALTH_URL total=$TOTAL_REQS conc=$CONCURRENCY pass≥${MIN_PASS_PCT}%"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/5] SKIP: CONFIG_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== Config DDoS Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Config DDoS Challenge: PASSED ==="
