...
# for Config per CONST-035 + CONST-050(B). Cascade per CONST-051(A).

set -uo pipefail
HEALTH_URL="${CONFIG_HEALTH_URL:-}"
CHAOS_HOST="${CONFIG_CHAOS_HOST:-localhost}"
CHAOS_PORT="${CONFIG_CHAOS_PORT:-}"
LEGIT_REQS="${CHAOS_LEGIT_REQUESTS:-100}"
MIN_PCT="${CHAOS_LEGIT_MIN_PASS_PCT:-95}"

echo "=== Config Chaos Failure-Injection Challenge ==="
echo "  url=$HEALTH_URL host=${CHAOS_HOST}:${CHAOS_PORT}"

if [[ -z "$HEALTH_URL" ]] || [[ -z "$CHAOS_PORT" ]]; then
    echo "[1/6] SKIP: CONFIG_HEALTH_URL/_PORT unset — SKIP-OK: #env-no-target"
    echo "=== Config Chaos Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Config Chaos Challenge: PASSED ==="
